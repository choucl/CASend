...
int accumulated_sz = 0;
int pbar_exit = 0;
void *progress_bar(void *argp) {
  size_t fsize = (size_t)argp;
  int bytes_per_block = fsize / BLOCK_NUM;
  int percentage = 0;
  char pbar_buf[BLOCK_NUM + 10];
  while (percentage < 100) {
    int block_cnt = accumulated_sz / bytes_per_block;
    percentage = accumulated_sz * 100 / (int)fsize;
    pbar_buf[0] = '[';
